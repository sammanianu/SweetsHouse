<?php include 'Templates/Header1.php' ?>

<br><br><br>
<div class="topic">
    <h2>-------------------------Birthday Cakes---------------------------------</h2>
</div>

<div class="row" >

    <div class="col-sm-5 col-md-4">
        <div class="col-sm-5 col-md-offset-1 col-md-11">
            <div class="gallery">
                <h3>BC001</h3>
                <a data-target="#myModal001" data-toggle="modal">
                    <img src="<?php echo base_url().'css/images/Birthday/bc001.jpg';?>" width="240" height="200">
                </a>
            </div>
            <br>
        </div>
    </div>

    <div class="col-md-4">
        <div class="col-sm-5 col-md-offset-1 col-md-11">
            <div class="gallery">
                <h3>BC002</h3>
                <a data-target="#myModal002" data-toggle="modal">
                    <img src="<?php echo base_url().'css/images/Birthday/bc002.jpg';?>" width="240" height="200">
                </a>
            </div>
            <br>
        </div>
    </div>

    <div class="col-md-4">
        <div class="col-sm-5 col-md-offset-1 col-md-11">
            <div class="gallery">
                <h3>BC003</h3>
                <a data-target="#myModal003" data-toggle="modal">
                    <img src="<?php echo base_url().'css/images/Birthday/bc003.jpg';?>" width="240" height="200">
                </a>
            </div>
            <br>
        </div>
    </div>
</div>
<br>
<div class="row">

    <div class="col-md-4">
        <div class="col-sm-5 col-md-offset-1 col-md-11">
            <div class="gallery">
                <h3>BC004</h3>
                <a data-target="#myModal004" data-toggle="modal">
                    <img src="<?php echo base_url().'css/images/Birthday/bc004.jpg';?>" width="240" height="200">
                </a>
            </div>
            <br>
        </div>
    </div>

    <div class="col-md-4">
        <div class="col-sm-5 col-md-offset-1 col-md-11">
            <div class="gallery">
                <h3>BC005</h3>
                <a data-target="#myModal005" data-toggle="modal">
                    <img src="<?php echo base_url().'css/images/Birthday/bc005.jpg';?>" width="240" height="200">
                </a>
            </div>
            <br>
        </div>
    </div>

    <div class="col-md-4">
        <div class="col-sm-5 col-md-offset-1 col-md-11">
            <div class="gallery">
                <h3>BC006</h3>
                <a data-target="#myModal006" data-toggle="modal">
                    <img src="<?php echo base_url().'css/images/Birthday/bc006.jpg';?>" width="240" height="200">
                </a>
            </div>
            <br>
        </div>
    </div>
</div>

<br>
<div class="row">

    <div class="col-md-4">
        <div class="col-sm-5 col-md-offset-1 col-md-11">
            <div class="gallery">
                <h3>BC007</h3>
                <a data-target="#myModal007" data-toggle="modal">
                    <img src="<?php echo base_url().'css/images/Birthday/bc007.jpg';?>" width="240" height="200">
                </a>
            </div>
            <br>
        </div>
    </div>

    <div class="col-md-4">
        <div class="col-sm-5 col-md-offset-1 col-md-11">
            <div class="gallery">
                <h3>BC008</h3>
                <a data-target="#myModal008" data-toggle="modal">
                    <img src="<?php echo base_url().'css/images/Birthday/bc008.jpg';?>" width="240" height="200">
                </a>
            </div>
            <br>
        </div>
    </div>

    <div class="col-md-4">
        <div class="col-sm-5 col-md-offset-1 col-md-11">
            <div class="gallery">
                <h3>BC009</h3>
                <a data-target="#myModal009" data-toggle="modal">
                    <img src="<?php echo base_url().'css/images/Birthday/bc009.jpg';?>" width="240" height="200">
                </a>
            </div>
            <br>
        </div>
    </div>
</div>


<br><br><br>












<!-- Modal -->
<div class="modal fade" id="myModal001" role="dialog">
    <div class="modal-dialog">

        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">BC001</h4>
            </div>
            <div class="modal-body">
                <img src="<?php echo base_url().'css/images/Birthday/bc001.jpg';?>" alt="Mountains" width="550" height="400">
                <p>Some text in the modal.</p>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="myModal002" role="dialog">
    <div class="modal-dialog">

        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">BC002</h4>
            </div>
            <div class="modal-body">
                <img src="<?php echo base_url().'css/images/Birthday/bc002.jpg';?>" alt="Mountains" width="550" height="400">
                <p>Some text in the modal.</p>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="myModal003" role="dialog">
    <div class="modal-dialog">

        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">BC003</h4>
            </div>
            <div class="modal-body">
                <img src="<?php echo base_url().'css/images/Birthday/bc003.jpg';?>" alt="Mountains" width="550" height="400">
                <p>Some text in the modal.</p>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="myModal004" role="dialog">
    <div class="modal-dialog">

        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">BC004</h4>
            </div>
            <div class="modal-body">
                <img src="<?php echo base_url().'css/images/Birthday/bc004.jpg';?>" alt="Mountains" width="550" height="400">
                <p>Some text in the modal.</p>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="myModal005" role="dialog">
    <div class="modal-dialog">

        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">BC005</h4>
            </div>
            <div class="modal-body">
                <img src="<?php echo base_url().'css/images/Birthday/bc005.jpg';?>" alt="Mountains" width="550" height="400">
                <p>Some text in the modal.</p>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="myModal006" role="dialog">
    <div class="modal-dialog">

        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">BC006</h4>
            </div>
            <div class="modal-body">
                <img src="<?php echo base_url().'css/images/Birthday/bc006.jpg';?>" alt="Mountains" width="550" height="400">
                <p>Some text in the modal.</p>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="myModal007" role="dialog">
    <div class="modal-dialog">

        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">BC007</h4>
            </div>
            <div class="modal-body">
                <img src="<?php echo base_url().'css/images/Birthday/bc007.jpg';?>" alt="Mountains" width="550" height="400">
                <p>Some text in the modal.</p>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="myModal008" role="dialog">
    <div class="modal-dialog">

        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">BC008</h4>
            </div>
            <div class="modal-body">
                <img src="<?php echo base_url().'css/images/Birthday/bc008.jpg';?>" alt="Mountains" width="550" height="400">
                <p>Some text in the modal.</p>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="myModal009" role="dialog">
    <div class="modal-dialog">

        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">BC009</h4>
            </div>
            <div class="modal-body">
                <img src="<?php echo base_url().'css/images/Birthday/bc009.jpg';?>" alt="Mountains" width="550" height="400">
                <p>Some text in the modal.</p>
            </div>
        </div>
    </div>
</div>